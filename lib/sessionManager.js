...
// Includes sendText/sendMedia, media download, and NEW contacts & lookup helpers.
...
export function createSessionManager({ dataPath = './.wwebjs_auth', registry }) {
  ...
  // Lookup list of numbers: is on WhatsApp? (getNumberId), then enrich + hasChat
  async function lookupContactsByNumbers({ accountId, label, numbers = [], countryCode = null, withDetails = true }) {
    ...
    for (const raw of numbers) {
      ...
      let numId = null;
      try {
        numId = await client.getNumberId(normalized); // returns null if not registered
      } catch {
        numId = null;
      }
      ...
      const waId = numId._serialized; // e.g. "5218115449142@c.us"
