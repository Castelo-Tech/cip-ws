...
// No message-sending endpoints here.  ➜ (now extended to support sendText, sendMedia, and on-demand media download)
...
export function createSessionManager({ dataPath = './.wwebjs_auth', registry }) {
  ...
  function bind(meta, client) {
    ...
    client.on('message_create', (m) => {
      // Keep a reference for later download if it has media
      ...
      // If it has media, emit a handy REST path the client can fetch when needed.
      // The HTTP server will enforce auth & ACL.
  ...
  function init(meta) {
    ...
    const executablePath = process.env.CHROME_PATH || undefined; // For video/gif (AAC/H.264) support (Chrome), else Chromium.
    const client = new Client({
      ...
      puppeteer: {
        args: pArgs,
        executablePath, // optional; set CHROME_PATH=/usr/bin/google-chrome-stable on Linux
      },
    });
    bind(meta, client);
  ...
  async function sendText({ accountId, label, to, text, options = {} }) {
    ...
    // MessageSendOptions is supported here, including linkPreview
    // Ref: docs sendMessage + options. (See citations below.)
    const msg = await client.sendMessage(chatId, String(text), options);
    // optionally emit a synthetic "outgoing" event
  ...
  // media: {mimetype, data(base64), filename?, filesize?} | {url} | {localPath}
  // options can include { caption, sendAudioAsVoice, sendMediaAsDocument, linkPreview, sendVideoAsGif, ... }
  async function sendMedia({ accountId, label, to, media, options = {} }) {
    ...
    // Voice note: supply sendAudioAsVoice: true and correct mimetype (audio/ogg; codecs=opus)
  ...
  // ---------- NEW: media download by messageId (lazy) ----------
  // Returns { mimetype, filename, dataB64 } or null if unavailable.
  async function downloadMessageMedia({ accountId, label, messageId }) {
    ...
    // downloadMedia() returns MessageMedia { mimetype, data (base64), filename }
    // Important: it can return undefined if media is no longer available.
