...
export function createSessionManager({ dataPath = './.wwebjs_auth', registry }) {
  ...
  async function downloadMessageMedia({ accountId, label, messageId }) {
    ...
    // Enrich a subset of contacts with profilePicUrl + about (sequential, jittered)
  async function enrichContacts({ accountId, label, contacts = [] }) {
    const client = await getReadyClient({ accountId, label });

    const out = [];
    let i = 0;
    for (const c of contacts) {
      // Only enrich private contacts with a WA id
      const waId = c?.id || null;
      if (!waId || c?.type === 'group') { out.push(c); continue; }

      // Light jitter + occasional longer breather to be gentle
      ...
      try {
        ...
        out.push({
          ...
          about: typeof about === 'string' ? about : (c.about ?? null),
        });
      } catch {
        out.push(c);
      }
