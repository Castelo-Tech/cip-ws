...
export function createSessionManager({ dataPath = './.wwebjs_auth', registry }) {
  ...
  // Build a best-effort numeric string from input and optional explicit CC. No assumptions if CC not provided.
  function buildRawNumber(input, countryCode) {
  ...
  async function lookupContactsByNumbers({ accountId, label, numbers = [], countryCode = null, withDetails = true }) {
    ...
    for (const raw of numbers) {
      ...
      const waId = numId._serialized; // e.g. "521811....@c.us"
  ...
  async function checkContactByNumber({ accountId, label, number, countryCode = null, withDetails = false }) {
    ...
    if (!numId?.serialized && !numId?._serialized) {
  ...
  async function getChatByNumber({ accountId, label, number, countryCode = null }) {
    ...
    let chat = null;
    try {
      const c = await client.getChatById(waId);
      if (c) chat = c;
    } catch {}
    if (!chat) {
      try {
  ...
  // NEW: bulk chats by numbers, optional last N messages
  async function getChatsByNumbers({ accountId, label, numbers = [], countryCode = null, withMessages = false, messagesLimit = 20 }) {
    ...
    for (const raw of numbers) {
      ...
      let numId = null;
      try {
        numId = await client.getNumberId(normalized);
      } catch {
        numId = null;
      }
      ...
      if (withMessages) {
        try {
          const list = await chat.fetchMessages({ limit: messagesLimit });
          messages = (list || []).map((m) => ({
            ...
            hasMedia: !!m?.hasMedia,
            // NOTE: media download via our /media/:messageId requires the message to be in cache from live events.
            // For history fetches we don’t attach a mediaUrlPath here to avoid broken downloads.
          })).filter((x) => x.id);
        } catch {
          messages = [];
        }
  ...
  return {
    ...
    // Contacts
    getContacts,
    lookupContactsByNumbers,
    checkContactByNumber,
