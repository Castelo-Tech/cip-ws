...
import { sleep, rand } from './session/utils.js';
...
export function createSessionManager({ dataPath = './.wwebjs_auth', registry }) {
  ...
  function init(meta) {
    ...
    // If CHROME_PATH is provided we use it; else Puppeteer Chromium.
    // This keeps your systemd deployment flexible.
  ...
  // ---------- Enrichment (sequential, moderate jitter) ----------
  // Fills profilePicUrl + about for given contacts, in order, with light random pauses.
  async function enrichContactsSequential({ accountId, label, contacts: list = [] }) {
    const client = await getReadyClient({ accountId, label });

    const out = [];
    let i = 0;
    for (const c of list) {
      const waId = c?.id || null;
      if (!waId || c?.type === 'group') { out.push(c); continue; }

      // Moderate jitter per contact; occasional short breather.
      await sleep(rand(45, 110));             // ~0.05–0.11s per contact
      if ((i++ % 25) === 0 && i > 1) {        // every 25 contacts, short pause
        await sleep(rand(200, 450));          // 0.2–0.45s
      }
      ...
      try {
        ...
        out.push({
          ...c,
          profilePicUrl: profilePicUrl ?? c.profilePicUrl ?? null,
          about: (typeof about === 'string' && about.length) ? about : (c.about ?? null),
        });
      } catch {
        out.push(c);
      }
  ...
  return {
    ...
    // NEW: sequential enrichment
    enrichContactsSequential,
